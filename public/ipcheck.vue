            ...
            <div class="form-check form-switch col-8 jn-radio">
                <div :class="{ 'col-4': isMobile }">
                    <input
                        ...
                        @click="$trackEvent('IPCheck', 'ToggleClick', 'Collaspes')"
                    />
                    <label v-if="isMobile" class="form-check-label" for="collapseSwitch">
                        &nbsp;
                        ...
                        <div class="card-body">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item jn-list-group-item" :class="{ 'dark-mode': isDarkMode }">
                                    <span class="jn-text">🖥️ {{ $t('ipInfos.IP') }}</span>
                                    : {{ card.ip }}
                                    <i
                                        v-if="isValidIP(card.ip)"
                                        :class="
                                            copiedStatus[card.id]
                                                ? 'bi bi-clipboard-check-fill'
                                                : 'bi bi-clipboard-plus'
                                        "
                                        @click="copyToClipboard(card.ip, card.id)"
                                    />
                                </li>
                                <li
                                    class="list-group-item jn-list-group-item"
                                ...
                                >
                                    ...
                                    <span
                                        ...
                                        :class="'fi fi-' + card.country_code.toLowerCase()"
                                    />
                                </li>
                                <li
                                ...
                                >
                                    ...
                                    :
                                    <a v-if="card.asnlink" :href="card.asnlink" target="_blank">{{ card.asn }}</a>
                                    <a v-else>{{ card.asn }}</a>
                                </li>
                            </ul>
                        </div>
...
export default {
    ...
    created() {
        // Bing Map functionality removed
    },
    ...
    methods: {
        ...
        // 从 PConline 获取 IP 地址（使用代理解决 CORS）
        getIPFromTaobao() {
            const url = '/api/pconline'
            fetch(url)
        ...
        getIPFromGCR() {
            ...
            fetch(url)
                ...
                .then((data) => {
                    ...
                    this.ipDataCards[1].source = 'IP.sb'
                    this.fetchIPDetails(1, ip)
                })
        ...
        async fetchIPDetails(cardIndex, ip) {
            ...
            const sources = [
                ...
                { url: `/api/ipapico?ip=${ip}`, transform: this.transformDataFromIPapi },
                { url: `/api/ipwhois?ip=${ip}`, transform: this.transformDataFromIPWhois }
            ]
